      ...
      <div id="requestMessages">
		...
		<div id="mainHolder">
	 		<div class="stickyContainer" id="dmTopBar">
				...
				<span class="topbarName">DoorDash</span>
			...
			<div id="dmHolder">
	 			<div id="dms">
		 			<div id="newConvoMessage">
						...
						<div class="newConvoName">DoorDash</div>
						<div class="newConvoText">Embark on an amazing conversation with <b>DoorDash</b>!</div>
	 				</div>
		 			<div class="dm">
						<img class="dmImage" src="https://photop-content.s3.amazonaws.com/ProfileImages/6066b99198895e660082965bcb91d776">
						<div class="dmContent">
							<div class="dmUser">
								<div class="dmUsername">DoorDash</div>
...
pages.messages = async function() {
  ...
  tempListen(findI("newMessage"), "click", async function () {
	...
	tempListen(document.querySelector(".sendDMButton"), "click", async function() {
		...
		const [code, response] = await sendRequest("POST", "conversations/messages/new" + `?convid=${activeMessage.getAttribute("convid")}`, formData, true);
		if(code == 200) {
			document.querySelector(".sendDMInput").innerText = "";
			if(!activeMessage.getAttribute("accepted")) {
				let [code2, response2] = await sendRequest("PUT", "conversations/requests/accept?convid=" + activeMessage.getAttribute("convid"))
				if(code2 == 200) {
					requestsOpen = false;
					startMessageBtn.innerText = "Start a Message";
					findI("activeMessages").style.opacity = 1;
